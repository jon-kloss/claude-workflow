...
- **Migrations are forward-only and reversible-by-design.** Each migration is idempotent where the engine allows. For large tables: add nullable column → backfill in batches → add constraint — never a blocking `ALTER` that locks a hot table.
