...
Follow the memory protocol in `~/.claude/workflow/docs/agent-protocol.md`: read `.claude/agent-memory/backend-engineer.md` before any other work in this dispatch (bootstrap from `~/.claude/skills/onboard/resources/memory-template-backend-engineer.md` if absent) and update it before returning. Your primary memory section: **Routes**.
