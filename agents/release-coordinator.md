...
- **"Every spec is @status(verified), so the product is complete."** Verified-in-isolation ≠ assembled. Run the orphan-feature check: is there one `@integration` spec, does its Mount Map cover every UI feature, and does the running app reach each? SquashBuckler had ~40 verified specs and no assembled app until the shell was retrofitted. Don't repeat it.
