...
- `@trivial` — optional flag for typo fixes, renames, and config-only changes. The single verification-scaling knob: set at decomposition only, never during /build. Permits skipping architecture docs, external feasibility research, and the tagged reviewer steps in /build. Mutually exclusive with multi-spec decompositions.
- `@touches-data` — optional flag indicating the spec adds, modifies, or migrates persistent data (schema changes, new tables/collections/columns, data migrations, new queries on hot paths). THE trigger for the data-architect role agent during /build Step 3.1 (investigation) and Step 3.3f (data-review). Apply during decomposition (Step 2.5) for any spec whose Technical Context includes schema or migration work, whatever its `@layer`. The hook warns — does not block — when an `@layer(api|full-stack)` spec lacks the tag; set it explicitly to be unambiguous.
- `@visual-pixel-diff` — optional. Opts the spec into pixel-diff visual fidelity comparison alongside the mandatory structural diff during /build Step 3.3g (qa-verification). Use ONLY for brand surfaces or exact-recreation specs (landing pages, marketing surfaces, design-token compliance reference pages). Pixel-diff is flaky on subpixel rendering and font hinting otherwise — the default structural diff is more stable.
...
- `@integration` — marks the single spec that ASSEMBLES all features into one running product (the app shell / entry point + primary navigation). **Required when an epic has ≥2 `@layer(ui|full-stack)` specs.** It `@depends-on` every UI feature and carries a `## Mount Map` section — a table with one row per UI feature: `| Feature spec | Mounts as (component) | Where (route / region / nav) |`. The Mount Map is the assembly contract; `require-feature-mounted.sh` reads it to gate `@status(verified)`, and qa-engineer Step 4.1 asserts every row is reachable from the entry point. Without an `@integration` spec, features ship as a launchpad of disconnected demo cards (see `docs/incidents.md#squashbuckler-2026-05-31`).
- `@mounts-in(integration-slug)` — on a `@layer(ui|full-stack)` feature: declares which `@integration` spec mounts it. Required on every UI feature in a ≥2-UI-spec epic. The feature must also appear in that integration spec's Mount Map.
- `@mount-skip(reason)` — override for `require-feature-mounted.sh`: this UI feature is legitimately not mounted in the shell (e.g. it is a sub-component another feature mounts, or a background-only surface). The reason persists as audit trail.
- `@integration-skip(reason)` — override: this epic legitimately has no single assembly owner despite ≥2 UI specs (rare). Document why.
